...
//协程与通信
//通过共享内存来通信，通信强制协作
//协程是根据一个或多个线程的可用性映射(多路复用，执行于)在他们之上的
//并行是通过多个处理器来提升速度，并发程序可能是并行的，也可能不是
//go协程意味着并行，协程一般来说不是
//go协程通过通道来通信，协程通过让出和恢复操作来通信
//数据通过通道：同一时间只有一个协程可以访问数据，所以不会出现数据竞争
//通道服务于通信的两个目的：值的交换，同步的，保证了两个协程任何时候都是可知状态
//通道实际上是类型化消息的队列
//ch1 := make(chan string) funcChan := chan func()
//ch <- int1 用通道ch发送变量int1 发送
//int2 := < -ch 变量int2从通道ch接收数据
//一个无缓冲通道只包含1个元素，同步通道-使用带缓冲的通道
//buf := 100 ch1 := make(chan string, buf) buf表示容纳的元素个数
//buf == 0 同步的，阻塞的 > 0 异步的，非阻塞
//for i, v := range data {
